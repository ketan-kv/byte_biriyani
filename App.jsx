...
function buildSystem(result) {
  if (!result) return "You are an expert data analyst assistant.";
  const domain   = result.domain ?? "unknown";
  const ov       = result.analysis?.overview ?? {};
  const insights = (result.insights ?? []).slice(0,5)
    .map(i=>`[${i.severity}] ${i.title}: ${i.explanation}`).join("\n");
  const corr     = (result.analysis?.correlations ?? []).slice(0,4)
    .map(c=>`${c.col_a}×${c.col_b} r=${+(c.correlation??c.r??0).toFixed(2)}`).join(", ");
  const kpis     = (result.knowledge?.kpis ?? []).slice(0,5)
    .map(k=>`${k.name}(${k.normal_range})`).join(", ");
  const story    = (result.executive_storyline ?? []).slice(0,2)
    .map(s=>s.narrative??s.text??"").join(" ");

  return `You are an expert ${domain} data analyst embedded in AMDAIS.

DATASET: ${fmt(ov.rows)} rows · ${ov.columns} cols · ${ov.numeric_columns} numeric · ${ov.categorical_columns} categorical
DOMAIN: ${domain} (${fmtPct(result.confidence)}) · missing strategy: ${result.analysis?.missing_strategy??"none"}
KPIs: ${kpis||"none"}
TOP CORRELATIONS: ${corr||"none"}
KEY FINDINGS:\n${insights||"none"}
SUMMARY: ${story||"not available"}

YOUR ROLE:
- Answer questions by reasoning over the context above. Be concise and analytical.
- When the user asks to change analysis parameters, respond with an ACTION block:
  ACTION:{"type":"rerun","preferences":{"missing_strategy":"median"}}
- Valid preference keys: missing_strategy (none|mean|median|zero|drop), focus_columns (array of col names).
- Never fabricate data not in the context. Use ${domain}-specific terminology.`;
}
  ...
  return (
    <>
      ...
      <div style={{ paddingTop:phase!=="upload"?50:0 }}>
        ...
        {phase==="pipeline" && <PipelineView file={file} result={result} loading={loading} stagesCompleted={stages}/>}
